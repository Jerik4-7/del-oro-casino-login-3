...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-c70fb47c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-c70fb47c">'+'<span class="tile-face-c70fb47c tile-front-c70fb47c"></span>'+'<span class="tile-face-c70fb47c tile-back-c70fb47c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c70fb47c")){return;}
var back=tile.querySelector(".tile-back-c70fb47c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-c70fb47c");if(isMine){tile.classList.add("boom-c70fb47c");}
if(dim){tile.classList.add("dimmed-c70fb47c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-c70fb47c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-c70fb47c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c70fb47c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-c70fb47c");void panelEl.offsetWidth;panelEl.classList.add("jolt-c70fb47c");}
endRound();}
